...
int iniciaMemLog(char * nome, int numURLs)
...
// Saida: nao tem
{
  ...
  int retprint = fprintf(ml.log, "%d", numURLs);
  erroAssert(retprint>=0,"Nao foi possivel escrever registro");
  return result;
}

int registraTempoRounds()
{
  struct timespec tp, tdif;
  int result = clock_gettime(ml.clk_id,&tp);
  

  clkDifMemLog(ml.currtime,tp,&tdif);
  ml.currtime = tp;
  ml.count++;

  int retprint = fprintf(ml.log," %ld.%.9ld",
                        tdif.tv_sec,tdif.tv_nsec);
  erroAssert(retprint>=0,"Nao foi possivel escrever registro");
  return result;
}

int registraTempoInter()
{
  struct timespec tp, tdif;
  int result = clock_gettime(ml.clk_id,&tp);
...
void getAvgRegistro(std::ifstream& arq)
{
	int numURL;double rounds;double inter; double total;
  double sumRounds = 0;double sumInter = 0;double sumTotal = 0;
	for (int i = 0; i < 10; i++)
	{
		arq >> numURL >> rounds >> inter >> total;
		sumRounds += rounds;
		sumInter += inter;
		sumTotal += total;
	}
	sumRounds = sumRounds/10;
	sumInter = sumInter/10;
	sumTotal = sumTotal/10;
	FILE* arqOut = fopen("guardarMedias.txt", "wt");
  fprintf(arqOut, "%d %lf %lf %lf", numURL ,sumRounds ,sumInter , sumTotal);
}